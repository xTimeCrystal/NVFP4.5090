...
#include "cutlass/epilogue/thread/activation.h" // <-- ADD THIS INCLUDE
...
struct ReLUSquared {
  ...
  T operator()(T const& arg) const {
    T val = arg > T(0) ? arg : T(0);
    return val * val;
  }
};

// Vectorized array specialization required by CUTLASS EVT
template <typename Element, int N, bool RegisterSized>
struct ReLUSquared<cutlass::Array<Element, N, RegisterSized>> {
  struct Params {};
  
  CUTLASS_HOST_DEVICE
  cutlass::Array<Element, N, RegisterSized> operator()(cutlass::Array<Element, N, RegisterSized> const& arg) const {
    cutlass::Array<Element, N, RegisterSized> result;
    for (int i = 0; i < N; ++i) {
      Element val = arg[i] > Element(0) ? arg[i] : Element(0);
      result[i] = val * val;
    }
    return result;
  }
...
// 1. Implementation for NVFP4 with ReLU
at::Tensor cutlass_sm120a_nvfp4_mm_relu(
  const at::Tensor &A,
  const at::Tensor &B,
  const at::Tensor &scales_A,
  const at::Tensor &scales_B,
  ...
  const std::optional<at::Tensor> &bias)
{
  ...
  // NEW: Insert ReLU before cast
  using ReluCompute = Sm90Compute<cutlass::epilogue::thread::ReLU, ElementAcc, ElementAcc, RoundStyle>;
  using EVT_Relu = Sm90EVT<ReluCompute, EVT1>;
  
  using EVT2 = Sm90EVT<Cast, EVT_Relu>; // cast
...
// 2. Implementation for NVFP4 with ReLU Squared
at::Tensor cutlass_sm120a_nvfp4_mm_relu_2(
  const at::Tensor &A,
  const at::Tensor &B,
  const at::Tensor &scales_A,
  const at::Tensor &scales_B,
  ...
  const std::optional<at::Tensor> &bias)
{
  ...
  // NEW: Insert ReLU^2 before cast
  using Relu2Compute = Sm90Compute<ReLUSquared, ElementAcc, ElementAcc, RoundStyle>;
  using EVT_Relu2 = Sm90EVT<Relu2Compute, EVT1>;
  
  using EVT2 = Sm90EVT<Cast, EVT_Relu2>; // cast
...
// 3. Register the new implementations
TORCH_LIBRARY_IMPL(gn_kernels, CUDA, m)
{
  ...
  m.impl("gn_kernels::cutlass_sm120a_nvfp4_mm_relu", &cutlass_sm120a_nvfp4_mm_relu); // <-- ADDED
  m.impl("gn_kernels::cutlass_sm120a_nvfp4_mm_relu_2", &cutlass_sm120a_nvfp4_mm_relu_2); // <-- ADDED
}